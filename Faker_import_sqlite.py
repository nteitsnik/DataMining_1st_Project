...
def generate_patients(num_patients):
    ...
    for _ in range(num_patients):
        ...
        patients.append({
            ...
            "BaselineMedications": random.choice(["Statins", "Beta Blockers", "ACE Inhibitors", "None"])
        })
...
x=clinical_measurements_df.columns
curs=sqliteConnection
curs.execute("""Create table if not Exists Clinical_Measurements (x);""")
clinical_measurements_final_df.to_sql('Clinical_Measurements',curs,if_exists='replace',index=False)
curs.commit()
sqliteConnection.close()

sqliteConnection = sqlite3.connect('SQLite_Python.db')
x=patients_df.columns
curs=sqliteConnection
curs.execute("""Create table if not Exists Patients (x);""")
clinical_measurements_df.to_sql('Patients',curs,if_exists='replace',index=False)
curs.commit()
sqliteConnection.close()

sqliteConnection = sqlite3.connect('SQLite_Python.db')
x=events_and_treatments_df.columns
curs=sqliteConnection
curs.execute("""Create table if not Exists Events_and_treatments (x);""")
clinical_measurements_df.to_sql('Events_and_treatments',curs,if_exists='replace',index=False)
curs.commit()
sqliteConnection.close()
